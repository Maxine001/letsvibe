  ...
  useEffect(() => {
    ...
    const fetchUserOnlineStatus = async () => {
      ...
      try {
        ...
        console.error("Exception fetching user online status:", e);
        setIsOnline(false);
      }
    };
    ...
    if (currentSideScreen.userId) {
      channel = DB.channel('public:users:id=eq.' + currentSideScreen.userId)
        .on(
          'postgres_changes',
          { event: 'UPDATE', schema: 'public', table: 'users', filter: `id=eq.${currentSideScreen.userId}` },
          (payload) => {
        ...
        .subscribe((status) => {
          if (status === 'SUBSCRIBED') {
            // User is subscribed to real-time updates
          }
        });
    }
    ...
    <div className="bg-secondary flex w-full items-center">
        ...
        <div className="flex items-center pl-0 md:pl-5 h-full absolute left-12 md:relative md:left-0">
          ...
          <div className="ml-4">
            ...
            <div className="flex gap-1 text-zinc-400">
              ...
              {!currentSideScreen.isGroup && isOnline && (
                <div className="flex items-center gap-1">
                  (<div className="h-3 w-3 bg-green-500 rounded-full opacity-90"></div>
                  <p className="text-green-500 text-sm opacity-90">Online</p>)
                </div>
              )}
            </div>
          </div>
      ...
      {!currentSideScreen.isGroup && (
        <div className="flex gap-6 z-5 pr-8 w-fit">
          <PhoneIcon
            className="cursor-pointer hover:bg-dark p-2 h-11 rounded-full"
            onClick={() => {
          ...
          />
          <VideoCameraIcon
            className="cursor-pointer h-11 p-2 rounded-full hover:bg-dark"
            onClick={() => {
              setCurrentSideScreen((curr) => {
